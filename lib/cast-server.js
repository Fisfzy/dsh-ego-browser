...
export function initCastServer(ctx) {
  ...
  // The web shell exposes `webServer`; other Web hosts expose `httpServer`.
  // Both expose the same .register API, so pick whichever is present.
  const server = ctx.webServer ?? ctx.httpServer;
