			...
			ctx.effect(() => {
				...
				const makeDraggable = (el) => {
					...
					const down = (e) => {
						...
						// A sub-control that is itself interactive (a real button/input)
						// must not start a drag — except when the drag handle itself is
						// that element (the FAB is a <button>, so clicking INSIDE it
						// should drag the ball, not be swallowed).
