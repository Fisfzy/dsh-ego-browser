...
function EgoBrowserTab(props) {
	...
	if (!currentSpace) body = h$1("div", { className: "dsh-ego-side-body" }, h$1("div", { className: "dsh-ego-side-empty" }, h$1("div", null, wt("noActivePages")), h$1("div", { style: { fontSize: "11px" } }, wt("noActiveHint"))));
		...
		}, wt("openExternal"))), liveImg, h$1("div", { className: "dsh-ego-side-livetitle" }, currentSpace.title || currentSpace.url || wt("newTab")), h$1("div", { className: "dsh-ego-side-liveurl" + (state.zoomHint ? " dsh-ego-side-hint" : "") }, state.zoomHint || currentSpace.url || "")));
	}
...
function mountSidebarTab(ctx, betterSidebar) {
	...
	// SESSION: the open is scoped to the CALLING session, so a background
	// conversation's tool call opens the Tab in ITS OWN sidebar instead of the
	...
	var openWatchTab = function(sessionId) {
		...
		autoOpened[key] = true;
		try {
			betterSidebar.openTab({ type: "ego-browser:watch" }, sessionId ? { sessionId: sessionId } : void 0);
		} catch (e) {}
	};
	(async function() {
