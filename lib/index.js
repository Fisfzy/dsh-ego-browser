...
function markEgoToolCall(sessionId) {
	toolCallCount += 1;
	const frame = `event: tool-call\ndata: ${JSON.stringify(sessionId ? { count: toolCallCount, sessionId } : { count: toolCallCount })}\n\n`;
 ...
 * `exec.agent` is typed `unknown` in this plugin's seam, so the read is
 * structural; a call with no initiating agent leaves the open unscoped.
 */
