...
export const inject = ['tools', 'subprocess', { webServer: 'optional', httpServer: 'optional' }];
...
export function apply(ctx, config = {}) {
    ...
    // The host service is `webServer` on the web shell, `httpServer` elsewhere.
    if (typeof ctx.webServer?.register === 'function' || typeof ctx.httpServer?.register === 'function') {
        try {
            initCastServer(ctx);
        }
        catch (err) {
