...
function registerActionTools(ctx, cfg, reg) {
  ...
  reg(
    t({
      name: "ego_page_info",
      description:
        "Return the current page info: url, title, viewport size (w, h), scroll offsets (sx, sy), and device metrics (pw, ph). Also reports when a native browser dialog is open.",
      parameters: {
        space: spaceParam,
      },
      buildScript: (args) =>
    ...
    defineTool({
      ...
      output: {
        schema: {
          type: "object",
          additionalProperties: false,
          properties: { topic: { type: "string", required: true }, text: { type: "string", required: true } },
        },
        render: renderText,
      },
      timeoutMs: 10_000,
