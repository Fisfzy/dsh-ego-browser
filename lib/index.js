...
function registerActionTools(ctx, cfg, reg) {
  ...
  reg(
    t({
      name: "ego_screenshot",
      description:
        "Capture a screenshot of the current page. Returns the file path of the saved PNG, which you can then read with a vision/image tool.",
      parameters: {
        space: spaceParam,
      },
      buildScript: (args) =>
  ...
  reg(
    t({
      name: "ego_key",
      description:
        "Press a keyboard key or shortcut combination on the current page, e.g. 'Enter', 'Tab', 'Control+a', 'Escape', 'ArrowDown'. Useful for forms, shortcuts and navigation.",
      parameters: {
        key: {
          type: "string",
          required: true,
          description:
            "Key or combo: 'Enter', 'Tab', 'Control+c', 'Meta+v', 'ArrowDown', 'Escape', 'F5', etc.",
        },
        space: spaceParam,
      },
      buildScript: (args) => {
        ...
        if (k === "")
          return `console.log('${SENTINEL}' + JSON.stringify({ ok: false, reason: 'ego_key: key is required' }))\n`;
        return (
          `${useSpace(str(args.space, cfg.defaultSpace))}${ensureRealTab()}` +
