...
function registerActionTools(ctx, cfg, reg) {
  ...
  reg(
    t({
      ...
      buildScript: (args) => {
        ...
        return (
          `${useSpace(str(args.space, cfg.defaultSpace))}${ensureRealTab()}` +
          `const __r = await cdp("Page.handleJavaScriptDialog", ${params}).catch(() => null)\n` +
          `const __info = await page.info()\n` +
          `console.log('${SENTINEL}' + JSON.stringify({ ok: true, handled: !!(__r && !__r.error), dialog: __info.dialog ?? null }))\n`
        );
      },
    })
  );
  reg(
    t({
      name: "ego_http",
      description:
        "Make a Node-side HTTP request (fetch.server, not from the browser page). Use to scrape an API, POST data, or hit a service without navigating. Returns status and body.",
      parameters: {
      ...
      buildScript: (args) => {
        ...
        // Pure Node-side request: no task space / browser tab needed.
        return (
          `${SAFE_FN}` +
          `const __r = await fetch.server(${j(str(args.url, ""))}, ${j(opts)})\n` +
          ...
          `console.log('${SENTINEL}' + JSON.stringify({ ok: true, status: __status, body: __body, url: ${j(str(args.url, ""))} }))\n`
        );
      },
    })
  );
