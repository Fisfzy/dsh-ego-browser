...
/**
 * Detect human-verification (CAPTCHA) on the current page. Serialized into the
 * page via page.evaluate. Returns { detected, kind } where kind is one of
 * 'recaptcha' | 'hcaptcha' | 'turnstile' | 'cloudflare' | 'captcha'.
 */
    ...
    t({
      ...
      buildScript: (args) => {
        const pts = Array.isArray(args.points)
          ? args.points.map(Number).filter((n) => Number.isFinite(n))
          : [];
        const hasEl =
...
const EGO_HELP_INDEX = {
  ...
  tools:
    "工具清单: ego_status, ego_health? no·, ego_space_open, ego_space_close, ego_snapshot, ego_navigate, ego_click(+double), ego_fill, ego_js, ego_cdp, ego_screenshot(+selector), ego_page_info, ego_wait, ego_wait_for_selector, ego_wait_for_url, ego_wait_for_response, ego_key(+text/type), ego_hover, ego_read_element, ego_select, ego_drag, ego_scroll, ego_upload, ego_check, ego_dialog, ego_http, ego_auth_flush, ego_help, ego_doctor, ego_cli, ego_script。",
  navigate: "ego_navigate: 打开URL或切tab(同任务复用当前tab)。ego_wait_for_url: 等跳转(登录/分页)。",
  observe:
    "ego_snapshot: 整页语义树(带[ref]/loc供点击); ego_page_info: url/标题/视口/滚动/对话框; ego_read_element: 读单元素文本/HTML/值/属性/可见性/计数; ego_screenshot(+selector): 整页或元素截图。",
...
function registerHelpAndDoctor(ctx, cfg, reg) {
  reg(
    defineTool({
      ...
      timeoutMs: 15_000,
      execute: async (args) => {
        // Reuse the action tool path so buildScript-based logic works identically.
        // Build a single call like the other tools.
        const { ok, value } = await (async () => {
          try {
            ...
            if (!result.ok) return { ok: false, value: { ok: false, detected: false, kind: null, error: result.error } };
            const p = parseSentinel(result.stdout);
            return { ok: true, value: p ?? { ok: false, detected: false, kind: null } };
          } catch (err) {
            return { ok: false, value: { ok: false, detected: false, kind: null, error: String(err?.message || err) } };
          }
        })();
        return {
          ok: !!value?.ok,
          detected: !!value?.humanCheck?.detected,
          kind: value?.humanCheck?.kind ?? null,
        };
      },
      ...
      execute: async () => {
        ...
        if (existsSync(bjson)) {
          try {
            ...
            const alive = b.pid ? await (async () => { try { process.kill(b.pid, 0); return true; } catch (x) { return x?.code === "EPERM"; } })() : false;
