...
import { access, chmod, copyFile, mkdir, mkdtemp, open, readFile, readdir, rename, rm, writeFile } from "node:fs/promises";
...
async function importLoginCookies(opts, deps) {
	...
	const browser = wanted ? browsers.find((b) => b.id === wanted) : browsers[0];
	if (!browser) return {
		ok: false,
	...
	const staging = await mkdtemp(join(tmpdir(), "ego-login-import-"));
	let tempHandle = null;
	let tempCdp = null;
	let egoCdp = null;
	try {
		await copyFile(join(browser.userDataDir, "Local State"), join(staging, "Local State"));
		const stagingProfile = join(staging, profile$1.dirName);
		await mkdir(join(stagingProfile, "Network"), { recursive: true });
		const cookieFiles = [
			[join(profile$1.dir, "Network", "Cookies"), join(stagingProfile, "Network", "Cookies")],
			[join(profile$1.dir, "Network", "Cookies-journal"), join(stagingProfile, "Network", "Cookies-journal")],
			[join(profile$1.dir, "Cookies"), join(stagingProfile, "Cookies")],
			[join(profile$1.dir, "Cookies-journal"), join(stagingProfile, "Cookies-journal")],
			[join(profile$1.dir, "Preferences"), join(stagingProfile, "Preferences")]
		];
		let copiedAny = false;
		for (const [from, to] of cookieFiles) try {
			await copyFile(from, to);
			if (from.endsWith("Cookies")) copiedAny = true;
		} catch {}
		if (!copiedAny) return {
			ok: false,
			error: `no cookie store found in profile "${profile$1.dirName}" (${profile$1.dir})`
		};
		tempHandle = deps.subprocess.spawn({
			argv: [
				browser.exePath,
				"--headless=new",
				`--user-data-dir=${staging}`,
		...
		tempHandle.done.catch(() => null);
		const portFile = join(staging, "DevToolsActivePort");
		let port = null;
		...
		if (port === null) return {
			ok: false,
			error: `${browser.label} headless instance did not expose a DevTools port within ${timeoutMs}ms (a running instance of the same browser may have refused the profile copy — close it and retry)`
		};
		tempCdp = await connectCdp(await browserWsUrl(port), 5e3);
		const got = await tempCdp.client.call("Storage.getCookies");
		const all = Array.isArray(got.cookies) ? got.cookies : [];
		...
		if (tempHandle) try {
			await Promise.race([tempHandle.done, sleep(4e3)]);
		} catch {}
...
function apply(ctx, config = {}) {
	...
	ctx.inject?.(["webServer"], (wctx) => {
		try {
			initCastServer(wctx, cfg, bridge, ffmpegManager, () => openAgentWindow(ctx, cfg));
		} catch (err) {
